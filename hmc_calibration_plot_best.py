...
def main():
    ...
    for section, basin in zip(sections["name"], sections["basin"]):
        ...
        file_name_sec = os.path.join(data_settings["data"]["station"]["folder"], data_settings["data"]["station"]["filename"]).format(domain=domain, section_name=section, section_basin=basin)
        if os.path.isfile(file_name_sec):
            section_data[section] = pd.read_csv(file_name_sec,
                                                sep=data_settings["data"]["station"]["sep"],
                                                usecols=[data_settings["data"]["station"]["date_col"],
                                                         data_settings["data"]["station"]["value_col"]],
                                                names=["date", "value"],
                                                index_col=["date"],
                                                parse_dates=True,
                                                date_parser=custom_date_parser,
                                                na_values=data_settings["data"]["station"]["null_values"]
                                                )[calib_hydro_start:calib_hydro_end].reindex(calibration_period,
                                                                                           method="nearest",
                                                                                           tolerance="1" + data_settings["data"]["station"]["calib_hydro_resolution"])
    ...
    resume_table = pd.DataFrame(index = section_data.keys(), columns=["nRMSE","MAPE","R2","CORR","NS","KGE"])
    for section in section_data.keys():
        fig, ax = plt.subplots(1,1)
        obs, = ax.plot(section_data[section].index, section_data[section].values, '-b')
        mod, = ax.plot(section_data[section].index, hmc_results[section].values, '-r')
        resume_table.loc[section,["nRMSE","MAPE","R2","CORR","NS","KGE"]] = [hm.nrmse_mean(hmc_results[section].values, section_data[section].values.squeeze()),
                                                                hm.mape(hmc_results[section].values, section_data[section].values.squeeze()),
                                                                hm.r_squared(hmc_results[section].values, section_data[section].values.squeeze()),
                                                                hm.pearson_r(hmc_results[section].values, section_data[section].values.squeeze()),
                                                                hm.nse(hmc_results[section].values, section_data[section].values.squeeze()),
                                                                hm.kge_2012(hmc_results[section].values, section_data[section].values.squeeze())]
